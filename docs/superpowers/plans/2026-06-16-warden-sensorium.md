# Provenance Sensorium Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and release Provenance Sensorium, a public-safe live-state awareness and provenance framework for AI-assisted research/security work.
...
- Create `src/provenance_sensorium/models.py`: status enums, provenance, observation, decision, receipt dataclasses.
- Create `src/provenance_sensorium/sensors.py`: file, git, claim, secret-shape, fixture sensors.
- Create `src/provenance_sensorium/layers.py`: secret, boundary, claim, and human-gate exception layers.
- Create `src/provenance_sensorium/receipts.py`: receipt assembly, JSON output, Markdown explanation.
- Create `src/provenance_sensorium/cli.py`: `scan`, `receipt`, `explain`, and `init-fixture` commands.
- Create `src/provenance_sensorium/public_surface.py`: release hygiene checks.
- Create `src/provenance_sensorium/__main__.py`: module entrypoint.
- Modify `src/provenance_sensorium/__init__.py`: package exports.
...
- Create: `src/provenance_sensorium/__init__.py`
- Create: `src/provenance_sensorium/__main__.py`
- Create: `.github/workflows/tests.yml`
- Create: `project-docs/public-boundary.md`

- [ ] **Step 1: Write package metadata**

Create `pyproject.toml` with package name `provenance-sensorium`, version `0.1.0`, Python `>=3.9`, console script `sensorium`, and pytest config pointing at `tests`.
...
Create empty exports in `__init__.py` and route `python -m provenance_sensorium` to `cli.main`.
...
```powershell
git add README.md LICENSE pyproject.toml src/provenance_sensorium/__init__.py src/provenance_sensorium/__main__.py .github/workflows/tests.yml project-docs/public-boundary.md
git commit -m "chore: scaffold Provenance Sensorium package"
```

## Task 2: Core Models

**Files:**
- Create: `src/provenance_sensorium/models.py`
- Create: `tests/test_models.py`
- Modify: `src/provenance_sensorium/__init__.py`
...
Expected: import failure for missing `provenance_sensorium.models`.
...
```powershell
git add src/provenance_sensorium/models.py src/provenance_sensorium/__init__.py tests/test_models.py
...
**Files:**
- Create: `src/provenance_sensorium/sensors.py`
...
```powershell
git add src/provenance_sensorium/sensors.py tests/test_sensors.py fixtures/sample_project
...
**Files:**
- Create: `src/provenance_sensorium/layers.py`
...
```powershell
git add src/provenance_sensorium/layers.py tests/test_layers.py
...
**Files:**
- Create: `src/provenance_sensorium/receipts.py`
- Create: `src/provenance_sensorium/cli.py`
- Modify: `src/provenance_sensorium/__main__.py`
...
```powershell
git add src/provenance_sensorium/receipts.py src/provenance_sensorium/cli.py src/provenance_sensorium/__main__.py tests/test_receipts.py tests/test_cli.py
...
**Files:**
- Create: `src/provenance_sensorium/public_surface.py`
...
```powershell
git add src/provenance_sensorium/public_surface.py scripts/check_public_surface.py tests/test_public_surface.py
...
**Files:**
- Modify: portfolio site landing and product pages.
...
```powershell
gh repo create HarperZ9/provenance-sensorium --public --description "Live-state awareness and provenance framework for AI-assisted research and security work." --source . --remote origin --push
```
...
```powershell
gh release create v0.1.0 --repo HarperZ9/provenance-sensorium --target main --title "Provenance Sensorium v0.1.0" --notes-file RELEASE_NOTES.md
```

- [ ] **Step 4: Promote on portfolio**

Update the live site to list Provenance Sensorium as a major product under AI safety / live-state awareness, not a small utility.
